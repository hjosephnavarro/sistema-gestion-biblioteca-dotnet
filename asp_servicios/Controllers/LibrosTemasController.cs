...
namespace asp_servicios.Controllers
{
    ...
    public class LibrosTemasController : ControllerBase
    {
        private readonly ILibrosTemasAplicacion _iAplicacion;
        private readonly TokenController _tokenController;

        public LibrosTemasController(ILibrosTemasAplicacion iAplicacion, TokenController tokenController)
        {
        ...
        public string Guardar()
        {
            try
            {
                ...
                var entidad = JsonConversor.ConvertirAObjeto<LibrosTemas>(entidadJson);

                _iAplicacion.Configurar(Configuracion.ObtenerValor("StringConexion") ?? string.Empty);

                //  Validación: no permitir duplicar la relación Libro + Tema
                var existentes = _iAplicacion.Listar();
                if (existentes.Any(x => x.Libro == entidad.Libro && x.Tema == entidad.Tema))
                {
                    return JsonConversor.ConvertirAString(new
                    {
                        Error = "Ya existe esta relación entre el libro y el tema."
                    });
                }
        ...
        public string Modificar()
        {
            try
            {
                ...
                // Validación: no permitir modificar hacia una relación duplicada
                var existentes = _iAplicacion.Listar();
                if (existentes.Any(x => x.Libro == entidad.Libro && x.Tema == entidad.Tema && x.Id != entidad.Id))
                {
                    return JsonConversor.ConvertirAString(new
                    {
                        Error = "Ya existe otra relación con el mismo libro y tema."
                    });
                }
