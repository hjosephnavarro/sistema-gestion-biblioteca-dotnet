...
namespace asp_servicios.Controllers
{
    ...
    {
        private readonly IExistenciasAplicacion _iAplicacion;
        private readonly TokenController _tokenController;

        public ExistenciasController(IExistenciasAplicacion iAplicacion, TokenController tokenController)
        {
        ...
        public string Guardar()
        {
            try
            {
                ...
                var entidad = JsonConversor.ConvertirAObjeto<Existencias>(entidadJson) ?? new Existencias();

                _iAplicacion.Configurar(Configuracion.ObtenerValor("StringConexion") ?? string.Empty);

                // Validar si ya existe una existencia del mismo libro
                var existentes = _iAplicacion.Listar();
                if (existentes.Any(e => e.Libro == entidad.Libro))
                {
                    return JsonConversor.ConvertirAString(new
                    {
                        Error = "Ya existe una existencia registrada para este libro."
                    });
                }
        ...
        public string Modificar()
        {
            try
            {
                ...
                // Validar duplicado al modificar
                var existentes = _iAplicacion.Listar();
                if (existentes.Any(e => e.Libro == entidad.Libro && e.Id != entidad.Id))
                {
                    return JsonConversor.ConvertirAString(new
                    {
                        Error = "Ya existe otra existencia registrada para este libro."
                    });
                }
